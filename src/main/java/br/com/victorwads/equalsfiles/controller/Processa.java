...
public class Processa implements Runnable, Duracao.Listenner {
	...
	public Processa(ProcessaInterface view, Resultado resultado) {
		this.view = view;
	...
	public void run() {
		...
		for (Diretorio diretorio : resultado.getDiretorios()) {
		...
		view.loading("Carregando Tamanhos", false);
		view.loading(0);
		...
		duracao.stop();
		resultado.setDuracao(duracao.getDuracao());
		resultado.setQuantidadeArquivos(arquivos.size());
		resultado.setTamanhoArquivos(totalArquivosTamanho);
	...
	private void compare(Arquivo a1, Arquivo a2) {
		...
		if ((!a1.getFlag()) && (!resultado.isNomeSensitive() || a1.nome.equals(a2.nome)) && (a1.getSize() == a2.getSize()) && getAquivoMD5(a1).equals(getAquivoMD5(a2))) {
			addArquivo(a1);
			addArquivo(a2);
			duplicatas++;
