...
public class Relatorio extends Conexao{
	...
	public boolean registrar(Resultado resultado){
		try{
			...
			ps.setLong(5, resultado.getData().getTime());
			...
			for(Map.Entry<String, ArrayList<Arquivo>> entry : resultado.entrySet()){
				for(Arquivo a : entry.getValue()){
					i = 0;
					ps = con.prepareStatement("INSERT INTO ResultadosArquivos (idResultado, diretorio, nome, tamanho, md5) VALUES (? ,? ,?, ?, ?)");
					ps.setInt(++i, resultado.getId());
			...
			result = result > 0 ? con.createStatement().executeUpdate("COMMIT") : 0;
	...
	public Resultado listar(int id){
		Resultado[] a = listar(null, null, id);
		if(a.length > 0)
			return a[0];
		else
			return null;
	...
	public Resultado[] listar(Date min, Date max){
		return listar(min, max, null);
	}

	private Resultado[] listar(Date min, Date max, Integer id){
		...
		try{
			...
			if(id != null){
				ps = con.prepareStatement(query + " WHERE id = ? LIMIT 1");
				ps.setInt(1, id);
